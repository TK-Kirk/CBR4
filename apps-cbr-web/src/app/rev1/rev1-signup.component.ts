...
export class Rev1SignupComponent implements OnInit {
  ...
  submit(): void {
    ...
    if (this.validateBirthdate() && this.contactform.valid) {
      ...
      const dob = `${this.contact.getBirthDateYear()}-${this.contact.getBirthDateMonth()}-${this.contact.getBirthDateDay()}`;
      this._rev1Service.postLead(this.contact)
        .subscribe((data: CoregLead) => {
          ...
          if (!data.isDuplicate) {
            ...
            this._router.navigate(['/rev1enter'], { queryParams: { 'email': this.contact.email, 'firstname': this.contact.firstname, 'lastname': this.contact.lastname, 'address': this.contact.address, 'zip': this.contact.zip, 'gender': this.contact.gender, 'birthdate': dob, 'country': this.contact.countryId, 'offerid': this.contact.offerId, 'affiliateid': this.contact.affiliateId, 'subid': this.contact.subId, 'cbrid': cbrId } });
          }
